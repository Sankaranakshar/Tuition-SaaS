# ClassStackr Execution Plan — R1 archive (✅ complete 2026-09-05)

> **This is a frozen archive.** R1 shipped and merged to `main` on 2026-09-05. It is
> kept for the per-step definition-of-done detail, the premise corrections found
> mid-step, and the "Step N" anchors that source-code comments and
> [EXECUTION_PLAN.md](../EXECUTION_PLAN.md)'s summary tracker still cite. **Do not
> edit it** — live planning is in [EXECUTION_PLAN.md](../EXECUTION_PLAN.md)
> (R2 scaffold), [MASTER_PLAN.md](../MASTER_PLAN.md), and
> [HANDOFF.md](../HANDOFF.md).

**What this was:** [MASTER_PLAN.md](../MASTER_PLAN.md) §3 R1 ("Money is correct") turned into an ordered sequence of steps small enough to execute one at a time, each with a concrete definition of done.

**Outcome:** all 13 steps done or explicitly founder-deferred (Steps 11 staging, 12 pentest/leaked-password), gate met at Step 13 (2026-09-05), stack merged to `main` via PRs #2/#3.
...
**One issue found during the re-walk — the picker leaks are now fixed (branch `fix-is-deleted-student-pickers`), the rest are deliberately left as-is:** `is_deleted = true` student rows — both archived students and B-11 erased stubs (name `"Erased student"`) — leaked into client-side `from("students").select(...)` sites that didn't filter them. Resolved per-site:
- **Filtered (they are active-list pickers): `CommandPalette.tsx` jump-to search, `Schedule.tsx` ClassWizard "Add Students", `Documents.tsx` upload picker.** `CommandPalette` and `Schedule` filter at the query; `Documents` filters at the `<option>` render so `getStudentName()` can still resolve names for an archived student's existing documents.
- **Left unfiltered on purpose:** `useMoney.ts:104` (name-resolution map for *retained* wallet rows — filtering would regress "Erased student" → "Unknown student"), `Today.tsx:233` (roster/name map — archived students can still be enrolled in a live session; archive doesn't touch `class_sessions`), `useStudentStory.ts` / `useInbox.ts` anchor context (load-by-id to render the "this record was erased/archived" state), `ParentPortal.tsx` / `usePeople.ts` family maps (built from `parent_links`, which erasure hard-deletes anyway). `useInbox.ts` messageable-contacts was already filtered. `Onboarding.tsx:436` is an `.insert()`, not a query.
Not an R1-gate blocker — erasure itself is correct (PII wiped, financial trail intact). All seven gates green after the fix; the three pickers verified live against production (the erased stubs no longer appear).
