...
> **This is a frozen archive, moved here from the repo root on 2026-09-12 and kept byte-identical (same section numbers) since.** Its still-genuinely-authoritative sections (§1, §3, §4, §12, §14) were distilled into [../MASTER_PLAN.md](../MASTER_PLAN.md) §12 — read that first for current market strategy. This file survives only because `server/utils/erasure.ts`, two migration files, and `shared/plans.ts` cite it by exact section number (§8.2 retention, §4.3 pricing). **Do not renumber or trim it** — that would break those citations. If you need to add or correct strategy content, do it in MASTER_PLAN.md §12, not here.
