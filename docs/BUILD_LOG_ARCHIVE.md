# ClassStackr Build Log Archive (§1 to §35, 2026-07-07 to 2026-07-25)

> **ARCHIVED 2026-07-25. This is history, not current state.**
>
> This was the original append-only HANDOFF.md. It grew to 1,383 lines across 35 sections, which made the current state hard to find, so it was replaced by a rewritten [HANDOFF.md](../HANDOFF.md) and [DEV_PLAN.md](../DEV_PLAN.md).
>
> **For current state, read those two files instead.** Every reusable lesson from the incidents below is distilled into the new HANDOFF's "Traps this stack has already sprung" section.
>
> Keep this file for one purpose: the full narrative behind a decision or a bug, when the distilled version is not enough. Statuses, test counts, and "next steps" throughout are frozen at the date each section was written and many are superseded.
>
> Section index: §8 security invariants, §11 Supabase migration, §13 to §14 Vercel deploy incidents, §16 dotenv + Realtime publication bugs, §17 founder deferral and playbook, §19 to §24 Stage 2 and Schedule builds, §25 full-repo audit, §27 to §33 Stage 3 items, §34 to §35 hardening gauntlet.
...
| §31 | Supabase Database Linter remediation + real schema-drift fixes (`rls_auto_enable` event trigger, quickstart `notes` table) — no live vulnerability found |
...
## 31. Supabase Database Linter remediation + schema drift cleanup (2026-07-19)

Ran the hosted project's Supabase Database Linter / Security Advisor cold (not tied to a specific feature) and worked every finding to ground truth against the actual codebase and live DB, rather than applying generic advice. Full per-finding writeup (risk rating, exploitability, exact SQL) lives in this session's chat log; summary here.

**Verdict: no live data-exposure vulnerability.** The 8 "RLS enabled, no policy" findings (`google_tokens`, `payment_gateways`, `parent_invites`, `student_invites`, `platform_admin_actions`, `invoice_counters`, `refunds`, plus `notes` — see below) are all correctly deny-all: every one is touched exclusively via `supabaseAdmin` (service_role) in `server/`, never from `src/`. Left as-is deliberately — adding client policies to silence the linter would have been a regression, not a fix.

**Fixed (`supabase/migrations/20260719140000_linter_hardening.sql`, pushed):**
- Pinned `search_path` on the two `BEFORE UPDATE` trigger functions that lacked it (`enforce_student_tutor_update_scope`, `enforce_profiles_org_immutable`, both from §rls-role-matrix/§profiles-immutable work). Both run `SECURITY INVOKER`, so this was never a live privilege-escalation path — closed anyway so the pattern doesn't get copied into a future `SECURITY DEFINER` function.
- Revoked `anon`/`authenticated` `EXECUTE` on `create_default_subscription()` and `students_enforce_plan_limit()` — both `RETURNS trigger`, only ever meant to fire via their triggers (subscription bootstrap, plan-cap enforcement), never callable as real RPCs regardless, but closed the dead PostgREST surface explicitly rather than relying on that implicit behavior.
- Left the six `is_org_member`/`has_role`/`is_staff`/`is_org_admin`/`is_parent_of`/`is_student_self` RLS-helper functions untouched even though the linter flags their `anon` RPC exposure — they're `SECURITY DEFINER` and correctly `search_path`-pinned already, return harmless `false` for `auth.uid() IS NULL` (anon), and revoking `authenticated` EXECUTE would break every RLS policy that calls them from `USING`/`WITH CHECK` — not worth the migration risk for a cosmetic finding.

**Real schema drift found and resolved — two objects existed on the hosted DB (`ref cwugpiernnwrhcximjwh`) with zero corresponding migration, meaning `supabase db reset` / a fresh local env would not reproduce production:**
1. **`rls_auto_enable()` + its `ensure_rls` event trigger** — turned out to be legitimate, deliberate infrastructure (auto-enables RLS on every new `public` table via `pg_event_trigger_ddl_commands()` on `ddl_command_end`) — this is the actual explanation for why RLS shows up deny-all on tables whose own migration never explicitly enables it. Already `SECURITY DEFINER SET search_path = pg_catalog` (safe). Captured verbatim into `supabase/migrations/20260719150000_capture_rls_auto_enable.sql` (no-op against the live DB, matches exactly) so local/staging parity holds going forward; also revoked its dead `anon`/`authenticated` EXECUTE grant in the same migration.
2. **`public.notes`** — a real 2-column, 3-row table, unreferenced anywhere in `server/`, `api/`, or `src/`. Row contents (`"Today I created a Supabase project." / "I added some data and queried it from Next.js." / "It was awesome!"`) confirmed it as leftover seed data from Supabase's own quickstart tutorial, not application data. Dropped via `supabase/migrations/20260719150100_drop_quickstart_notes.sql` after showing the founder the actual row contents before dropping (destructive action, confirmed first).

**Verification method:** used `supabase migration list --linked` before/after each push to confirm remote state matched local migrations exactly (no partial-apply risk); direct read-only `psql`/SQL-editor queries against the live DB (via the founder, since this session's safety classifier correctly declined running ad hoc queries against production directly) confirmed both drifted objects' real definitions before writing any fix, rather than guessing from the linter's one-line description.

**Not fixed — requires a dashboard action, not SQL:** leaked password protection (HaveIBeenPwned check on signup/password-change) is still disabled. Supabase Auth setting, not expressible in a migration — Dashboard → Authentication → Policies → enable "Leaked password protection." Real risk (credential stuffing via reused breached passwords), 2-minute fix, still outstanding.

**Next:** the leaked-password-protection dashboard toggle (someone with dashboard access, not scriptable), then back to the hardening gauntlet (DEV_PLAN §5/§9) where §30 left off.

---

## 32. Audit against the original 24-week plan (2026-07-20)

A user-requested comparison of everything actually built against the original week-by-week plan (the 24-week, 20-epic document that preceded this Supabase-era DEV_PLAN.md — its content is quoted in this session's chat log, not a separate file in the repo). Purpose: catch anything the epic-renumbering across the Firestore→Supabase rewrite silently dropped, as distinct from things deliberately deferred. Two real gaps found; everything else maps cleanly.

**Stage 0 (original Epics 1–5): done, but on different infrastructure than specified.** The plan's E1.4/E1.5 (Firebase emulator suite, three separate Firebase projects for dev/staging/prod) and all of Epic 2's tasks (E2.1–E2.8, Firestore rules + `@firebase/rules-unit-testing`) were written against Firestore and never executed as written — the whole stack migrated to Supabase/Postgres partway through Stage 0 (§11). The *intent* of every task (RBAC matrix as an executable suite, C1–C5 fixes, custom-claims-free role checks) landed via Postgres RLS + the PGlite `test:rls` suite instead. Nothing here is a gap; it's a documented architecture pivot, not a dropped task.

**Stage 1 (original Epics 6–10):**
- Epic 6 (Payments): built. Note the plan's own acceptance criteria assumed live-mode Razorpay throughout (E6.1's "test payment marks invoice paid" is the only test-mode-scoped one) — everything from E6.2 onward has only ever been exercised in test mode, not live. Not a gap; live-mode is founder-deferred (§17.1).
- Epics 7 (outbound comms) and 8 (Calendar/Meet): the plan's own week-5-day-1 founder task list (Razorpay live KYC, WhatsApp template approval, SMS DLT registration, CA invoice review, legal docs) was never actually started — these were meant to run in parallel with engineering from day one specifically so Epics 7/8 wouldn't still be blocked by week 6–8. They remain exactly as blocked today as they were the day they were deferred. This is a real schedule slip on the founder side, not an engineering gap — flagging it here since nothing in HANDOFF before this section called it out explicitly.
- Epics 9 (Today) and 10 (Parent portal): built, live-verified except the two items that were always going to need a live phone/Razorpay (E10.2's 375px device test, E10.3's real-UPI-on-a-phone criterion) — both still open, both externally blocked, unchanged from prior sections.
- Stage 1 exit gate ("the wedge demo... in live-mode Razorpay"): ran (§16, §18), but in test mode, not the live-mode Razorpay the gate's acceptance text specifies. Recorded here as a weaker pass than the original gate's letter, not a failure — the substance (attendance → invoice → link → payment → reconcile → Pulse, one unbroken take) is confirmed.

**Stage 2 (original Epics 11–14): done, matches the plan cleanly.** All four workspaces shipped, all listed legacy pages deleted in the epics that replaced them, exit gate cleared (§19–§23).

**Stage 3 (original Epics 15–17): 3 of 4 sub-epics done; two real gaps found here.**
- Epic 15 (Schedule): done (§24).
- Epic 16 (Operate-the-business layer): E16.1 (subscription billing, §27), E16.2 (super-admin console, §28), E16.3 (org export/offboarding, §30) all built and browser-verified. **E16.4 (audit log viewer) was never built and is not tracked anywhere else in this document or DEV_PLAN.md — a genuine gap, not a deferral.** `audit_events` has been written to by every privileged mutation since Epic 3 (§8 invariant #2), but nothing has ever surfaced those rows in a UI for either super-admin or org-admin to read. The plan's own acceptance criterion for Epic 16 — "you can answer a support ticket without touching the database console" — is not met for anything audit-trail-shaped; a support question like "who voided this invoice and when" currently requires a direct DB query. Scope, if picked up: a read-only table view over `audit_events`, filterable by org/actor/entity/date, gated the same way §28's org health view is (super-admin sees all orgs, org-admin sees only their own org's rows) — no new write path needed, the data already exists.
- Epic 17 (hardening gauntlet): **not started at all**, confirmed by grepping this file and DEV_PLAN.md for pentest/axe/k6 — no hits outside the planning sections themselves. The one exception is backup/restore (part of E17.4), which got done opportunistically during the MVP-launch-tasks pass, ahead of schedule and outside this epic's own sequencing — see DEV_PLAN §3 Critical. Load test (E17.1), external pentest (E17.2), accessibility pass (E17.3), and the docs/legal package beyond what HANDOFF itself already provides (E17.5) are all still open. This matches DEV_PLAN's own "Stage 3, rest — hardening gauntlet" ACTIVE flag; nothing new here, just confirming the flag is accurate against the original epic list.

**Stage 4 (original Epics 18–20): not started**, confirmed against DEV_PLAN §5's mobile polish / growth loop / AI morning brief / reporting bullets, all still unchecked.

**Net finding:** the build is further along than the calendar would suggest (roughly through the original plan's week-19 mark of scope, in about two weeks of actual elapsed time, since this was agent-driven rather than the two-human-engineer team the plan's week counts assumed) and every deviation traces to either a deliberate, already-documented decision (the Supabase migration, the founder's external-integrations deferral) or an already-flagged item (Epic 17). The two things that were **not** previously flagged anywhere and are net-new findings from this pass: **Epic 16.4's audit log viewer (never built, no tracking issue)** and **the founder's own week-5 long-lead external tasks never having been started**, which is why Epics 7/8 are exactly as blocked today as the day they were first deferred.

---

## 33. Tech Debt #31 / old Epic 16.4 — audit log viewer (2026-07-20)

Picked up the first of the two gaps §32 surfaced. `audit_events` has had real rows since Epic 3 (every privileged mutation writes one — §8 invariant #2) but nothing ever read them back in a UI. Scope was exactly what DEV_PLAN's Tech Debt #31 entry specified: a read-only table view, no new write path.

**Server:** new `GET /api/v1/audit-log` (`server/routes/auditLog.ts`, contract in `shared/schemas/auditLog.ts`). Unlike every route in `admin.ts`, this one is not platform-admin-only — it has two access tiers on the *same* endpoint, deliberately mirroring `audit_events_select`'s own RLS policy (`20260709021100_rls_role_matrix_fixes.sql`, `has_role(organization_id, array['owner','admin','accountant'])`) instead of inventing a new rule:
- A `platform_admins` allowlist hit (checked the same way `requirePlatformAdmin` does) sees every org, with an optional `?orgId=` filter.
- Anyone else needs `req.user.organizationId` set and their org role in `{owner, admin, accountant}`; the query is always pinned server-side to their own org — a client-sent `orgId` is silently ignored for this tier, not just unauthorized.

Filters: org (platform-admin tier only), `entityType` (matched against `payload->>'entityType'`, since `audit_events` doesn't have a dedicated column — `writeAudit()` has always folded `entityType`/`entityId` into the jsonb payload), `from`/`to` date range, `limit`/`offset` pagination (default 50, max 200). Joins `organizations` for the org name and `profiles` for the actor's name/email so the UI never has to show a bare uuid.

**Client:** `src/pages/AuditLog.tsx`, following `PlatformAdmin.tsx`'s existing page pattern (single component, no pure-core/hook split — this is an admin-tools page, not a Stage 2/3 workspace) rather than inventing a new one. Table with filter row above it, click-to-expand rows showing the full JSON payload, Previous/Next pagination. New rail icon (clipboard) in `Layout.tsx`, gated client-side by `isStaff (!isStudent && !isParent) || isPlatformAdmin` — same caveat as the existing platform-admin icon: this only controls whether the icon renders, the server route is the real boundary. Routed at `/app/audit-log` in `App.tsx`.

**No new table, no new RLS policy, no new pure logic** — this reads an existing table under gating equivalent to its existing RLS policy, so RLS suite stayed at 80/80 and unit suite stayed at 158/158 with no additions needed.

**Verified:** `tsc --noEmit` clean, 158/158 unit, 80/80 RLS, build passing, bundle-size gate green (`AuditLog-*.js` is its own 5.26 KB lazy chunk, not in the 224.6 KB gzip main entry — budget 260 KB). Browser-verified live against the real Supabase project (`npm run dev:preview`, logged in as the same platform-admin/tutor account used throughout this build): the page loaded all 39 real audit events across every org with actor names and org names correctly resolved; the entity-type filter (`invoices`) correctly narrowed to 4 matching events; the organization filter (`Demo Tuition Center`) correctly narrowed to that org's events only; clicking a row expanded it to show the full JSON payload (confirmed on a real `template.update_scope` event, showing the session-materialization `created`/`conflicts` detail). Did not have a second non-platform-admin login on hand to click-verify the 403 path for a user outside `{owner, admin, accountant}` — that path is covered by code review only (the same `ORG_SCOPED_ROLES` check pattern as `requireRole`), not a live walkthrough.

**Net effect:** DEV_PLAN Tech Debt #31 closed; Stage 3's only remaining item is now the hardening gauntlet (old Epic 17 — see DEV_PLAN §5/§9, still not started).

---

## 34. Hardening gauntlet, part 1 — supertest route-contract suite for billing/scheduling/parents (2026-07-25)

Old Epic 17's first piece (DEV_PLAN §9): route-contract tests. §9 had flagged this as a correction to prior HANDOFF sections — `supertest`/`@types/supertest` were installed devDependencies with zero actual tests. Scope, per DEV_PLAN's own framing: cover every privileged endpoint's auth matrix (401/403/200/409/422) against a PGlite-backed test app, starting with billing/scheduling/parents.

**Why this needed real infrastructure, not just test files.** The RLS suite (`tests/integration/`) tests RLS policies directly against PGlite with raw SQL — it never goes through the Express app, so it can't see auth-matrix behavior (role checks, zod validation, business-logic status codes) at all. Route-contract tests need the *real* `server/app.ts` running against something PGlite-shaped. Two things stood in the way:
- `pool`/`withTransaction` (`server/db.ts`) is a real `pg.Pool` on `DATABASE_URL` — routes import it directly at module load time.
- `supabaseAdmin` (`server/supabaseAdmin.ts`) is a real `@supabase/supabase-js` client that talks to PostgREST over HTTP — no PostgREST exists in this test setup.

**Built `tests/contract/`:**
- `pgliteBackend.ts` — a settable singleton holding the current PGlite instance, so `vi.mock` factories (which hoist above imports and can't close over a per-test `beforeAll` variable) have something to delegate to.
- `dbShim.ts` — `pool`/`withTransaction` reimplemented as thin wrappers over the PGlite instance (plain `begin`/`commit`/`rollback` text queries; PGlite is a single in-process engine, so this only holds up for sequential, non-concurrent test requests — real concurrency belongs in the k6 layer, not here).
- `supabaseShim.ts` — a from-scratch, intentionally minimal supabase-js query-builder compatible with exactly the method chains this codebase's routes actually use (grepped across `billing.ts`/`parents.ts`/`members.ts`/`middleware/auth.ts`: `select`/`insert`/`update`/`upsert`/`eq`/`limit`/`maybeSingle`/`single`, plus the builder being thenable). One special case: `organization_members`'s embedded `organizations(status)` select (the only nested-resource query anywhere in the codebase, used by `authenticateToken` on every request) gets an explicit LEFT JOIN rather than a generic embed parser. Unrecognized shapes throw loudly instead of silently returning wrong data.
- `setup.ts` — registers both `vi.mock`s once for the whole run via `vitest.contract.config.ts`'s `setupFiles`.
- `testApp.ts` — boots a fresh PGlite (`tests/integration/db.ts`'s `bootDb()`, migrations + auth shim, same as the RLS suite), seeds the RLS suite's own fixtures (`tests/integration/fixtures.ts` — same org, same users, same ids, so both suites share one mental model), points the shims at it, and dynamically imports `createApp()` so the mocks are already in place. Every query runs at PGlite's default bootstrap-superuser role (no `set local role` needed), matching the real trust boundary: a direct-Postgres `DATABASE_URL` connection and the service-role `supabaseAdmin` client both bypass RLS in production too — RLS itself stays the RLS suite's job.
- JWT signing (`jsonwebtoken`, HS256, matching `authenticateToken`'s real verification path) with a fixed test secret injected via `vitest.contract.config.ts`'s `env` block.

**A real, useful bug this surfaced in the test harness itself, not the app:** PGlite's `Results.affectedRows` is `0` for a `SELECT` (it's an "how many rows changed" count, not "how many came back"). The shim's first draft did `rowCount: res.affectedRows ?? rows.length` — since `??` doesn't treat `0` as nullish, every `SELECT` reported `rowCount: 0` regardless of how many rows it actually returned, so every one of this codebase's `rowCount === 0` existence checks (grepped: all of them are against `SELECT`s) was permanently and silently true. Fixed to always use `rows.length`, matching what node-postgres actually returns for a `SELECT` in production.

**A second, genuinely informative discovery, not a bug:** the RLS suite's own fixture ids (`tests/integration/fixtures.ts`'s `uids.*`/`ids.*`, e.g. `20000000-0000-0000-0000-000000000001`) are valid Postgres uuids but not valid RFC 4122 v1-8 uuids (version/variant nibbles are `0`) — fine for the RLS suite (raw SQL, no zod in the path) but rejected by every `z.string().uuid()` request-body field here. Route-contract tests insert their own real (`crypto.randomUUID()`) entities for anything referenced from a request body; fixture ids remain fine for path params and JWT actor ids, which zod never touches.

**Coverage:** `npm run test:contract` (new script, `vitest.contract.config.ts`), 70 tests, all passing on the first real run after the harness itself was debugged:
- `scheduling.test.ts` (17) — all 6 endpoints: enrollments (capacity, cross-org, 404/403/409), sessions (create, overlap conflict), reschedule (404/200/403), template scope update (403/200/404), gaps (422/200).
- `parents.test.ts` (17) — all 3 endpoints: invite create (role gate, no-org, validation, 404, 201), invite preview (401/404/410 expired/410 used/200), redeem (401/422/404/409 org-conflict/200 with real `parent_links` + `organization_members` + invite-burn assertions/410 double-redeem).
- `billing.test.ts` (36) — all 12 endpoints: invoices, wallet top-up (idempotency replay verified against the real ledger balance), attendance (future-session/cross-tutor/happy path), session cancel, manual payments (idempotency replay), void, finalize (idempotent renumber), payment-link + parent pay (both exercised through their real `gateway_not_connected` degraded path — the fixture org has no encrypted Razorpay creds, so this is the actual production behavior for any org that hasn't connected a gateway, not a mock standing in for one), invoice PDF (401/404/200 with a real rendered PDF buffer), refunds, reconcile (403/422 degraded).

**Verified:** all five gates green together — `tsc --noEmit` clean, 158/158 unit, 80/80 RLS, 70/70 route-contract, build passing, bundle-size unaffected (server-only test code, no client bundle impact).
