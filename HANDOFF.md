...
| **Stage 3, rest** | **Super-admin console, org export, hardening gauntlet** | **⬅ ACTIVE — this is the next work** (DEV_PLAN §5) |
...
**Verification status:** all gates green as of the 2026-07-19 subscription-billing build (§27) — `tsc --noEmit` clean · 143/143 unit · 69/69 RLS · build passing · bundle 223.7 KB gzip (260 KB budget). Live at `https://tuition-saas-two.vercel.app` against Supabase Cloud `cwugpiernnwrhcximjwh`; `git push` to `main` auto-deploys. **Two migrations are committed but not yet pushed to the hosted project** — see §26/§27's "known gap": this sandbox couldn't reach the hosted DB (`supabase projects list` showed it `INACTIVE`). Someone with real DB egress needs to run `supabase db push` and confirm the project isn't actually paused.
