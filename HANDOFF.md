...
_Last verified 2026-09-05 against the top of the R1 branch stack (`r1-step-10-dpdp-consent-erasure`, commit `1817bc1`, working tree clean; stack = step-10 → step-9 → steps-6-7 → `main`, none merged) as part of EXECUTION_PLAN.md **Step 13, the R1 gate checkpoint**. All seven gates re-run clean off the top of the stack; the money-touching flows (attendance reversal end-to-end, `/reconcile-wallets` + `/expire-credits` against production, bulk import, parent top-up degradation, per-student erasure) re-walked live against the production Supabase project — see §9. **R1 is complete** (MASTER_PLAN.md §3); the two founder-deferred gate lines — staging (B-10) and external pentest / leaked-password toggle — are recorded as explicit non-failing deferrals in EXECUTION_PLAN.md Step 13. Every number below was re-run, not inherited._
