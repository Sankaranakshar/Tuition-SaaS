...
**Verification status:** all gates green as of the 2026-07-19 org-export build (§30) — `tsc --noEmit` clean · 158/158 unit · 80/80 RLS · build passing · bundle 224.3 KB gzip (260 KB budget). Live at `https://tuition-saas-two.vercel.app` against Supabase Cloud `cwugpiernnwrhcximjwh`; `git push` to `main` auto-deploys (migration pushed via `supabase db push`, not yet committed/deployed to Vercel — see §30). **Resolved 2026-07-19 (§29):** `platform_admins` seeded with the founder's row; both §27 (subscription billing) and §28 (super-admin console) walked through live in a real browser against the hosted project, including the impersonation magic-link round-trip.
...
**Migration status:** `20260719130000_org_offboarding.sql` has been pushed to the hosted project via `supabase db push` (confirmed in `supabase migration list`). **The code itself has not yet been committed or pushed to `main`** — per this repo's standing rule, that needs the user's explicit go-ahead first (asked for and given for the `db push` specifically; not yet asked for the code commit/push).
