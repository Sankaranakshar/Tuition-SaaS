...
**Verification status:** all gates green as of the 2026-07-19 super-admin build (§28) — `tsc --noEmit` clean · 153/153 unit · 75/75 RLS · build passing · bundle 224.0 KB gzip (260 KB budget). Live at `https://tuition-saas-two.vercel.app` against Supabase Cloud `cwugpiernnwrhcximjwh`; `git push` to `main` auto-deploys. **Three migrations are committed but not yet pushed to the hosted project** — see §26/§27/§28's "known gap": this sandbox couldn't reach the hosted DB (`supabase projects list` showed it `INACTIVE`). Someone with real DB egress needs to run `supabase db push`, confirm the project isn't actually paused, and (for §28) seed at least one `platform_admins` row so someone can actually reach the console.
...
**Verification: all four gates green.** `tsc --noEmit` clean · **143/143 unit** (was 132, +11) · **69/69 RLS** (was 58, +11) · build passes · bundle-size **223.7 KB gzip** (budget 260 KB, effectively back to the pre-feature 223.0 KB baseline after the zod-split fix above). **Not yet done:** the new migration hasn't been pushed to the hosted Supabase project (same DB-egress gap as §26 — `supabase db push` needs to run from a machine/environment that can actually reach the project once its paused/inactive status is resolved), so `subscriptions` rows don't exist for real orgs yet and the enforcement trigger isn't live in production. No browser walkthrough — needs the live DB. Platform Razorpay account itself is out of scope per §17.1 (deferred to go-to-market); the checkout/webhook code is complete and inert, matching the existing degradation pattern used everywhere else in this codebase.
...
**Verification: all four gates green.** `tsc --noEmit` clean · **153/153 unit** (was 143, +10) · **75/75 RLS** (was 69, +6) · build passes · bundle-size **224.0 KB gzip** (budget 260 KB — the rail's new eager `useIsPlatformAdmin()` import in `Layout.tsx` adds a small amount to the always-loaded shell; the admin page itself is lazy-loaded and doesn't count against this number). **Not yet done:** same DB-egress gap as §26/§27 — the migration hasn't reached the hosted project, so `platform_admins` doesn't exist there yet and nobody can actually be granted access until it does (and until someone runs a one-off `insert into platform_admins (user_id) values ('<founder-uid>')` as `service_role`, since there's deliberately no self-service admin-granting UI). No browser walkthrough — needs the live DB plus a seeded platform-admin row. The impersonation magic-link flow specifically also needs GoTrue's email delivery *not* to matter here (the link itself carries the session grant, no email click required — the platform admin uses the returned `action_link` directly), but this has never been exercised against real GoTrue, only reasoned about from the API contract.

**Next:** org export/offboarding (JSON/XLSX, 8-year financial retention) → the hardening gauntlet (supertest route contracts, dependency audit — DEV_PLAN §5/§9).
