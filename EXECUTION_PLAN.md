...
**R1 and R2 are both fully closed** (all gate criteria met, Steps 1-21). What's below is real, ranked work that was explicitly left unscoped rather than done — not a new discovery. Use MASTER_PLAN.md §4's ranked backlog to break ties; do not assume the order below is final without re-checking that table, since a founder decision or a newly-shipped dependency can reorder it (as B-08 itself did — it scored lower than B-12 but was picked first because it closed R2's own gate, not because of raw score. R2's gate is now closed, so that override no longer applies and score should drive the next pick).

| ID | Item | Score | ed | Depends on | Notes |
|---|---|---|---|---|---|
| B-12 | Monthly progress-report PDF | 0.80 | 3 | — | Highest-scoring unscoped item as of 2026-09-12. No known blocker — a reasonable default next pick. Needs scoping: what data it pulls (attendance? assessments? both?), who can generate/download it (parent-facing? staff-facing? both?), and whether it reuses `invoicePdf.ts`'s `jsPDF`/`jspdf-autotable` pattern (likely yes, per this repo's convention of one composer style per document type). |
| B-13 | Substitute and leave management | 0.53 | 4 | Step 15 (done) | Unblocked, not yet scoped. Touches `class_sessions.tutor_id` reassignment and `tutor_availability` — read both before scoping, and check whether B-07's cross-org conflict check (Step 20, tutor-scoped not org-scoped) needs to account for a substitute's own schedule too. |
| — | Assignment-marking loop into the gradebook (unscored, IA-tab item) | — | — | — | "Upload works, marking doesn't, on both the staff and student side" per MASTER_PLAN.md §3's R2 IA-tabs note. Not yet investigated against the real schema — confirm what "upload" currently touches before assuming the fix is symmetrical. |
| — | Guardian records: student-owned → parent-owned (unscored, IA-tab item) | — | — | — | A data-model/ownership change (`students` table currently owns parent_name/parent_phone/parent_email as free-text fields — check whether this means moving them onto `parent_links`/`parent_profiles` instead, or something narrower). Not yet scoped. |
| — | Cross-org family view for parents (unscored, IA-tab item) | — | — | B-07 (done) | Now unblocked since the org switcher (Step 20) exists — a parent with children at two different centres should see one home screen instead of switching orgs manually. Not yet scoped; likely the biggest lift of the three IA items since it may need a genuinely cross-org query path, unlike everything built so far which is single-active-org-scoped by design. |
...
**Status as of 2026-09-12:** R1 and R2 are both complete and merged to `main` (Steps 1-21, all seven gates green: 228 unit / 100 RLS / 298 contract / build / bundle 204.7 KB / API bundle 17/17). The most recent work was B-08 (tutor payouts, Step 21) and a same-day fix to a real `Today.tsx` role-vs-organizationRole bug found while testing it. Nothing is currently in progress or broken. The next unscoped item, by MASTER_PLAN.md §4's ranked backlog, is **B-12 (Monthly progress-report PDF)** — see "R2 backlog" above.

**Prompt for a new session — paste this in:**

> Read HANDOFF.md's intro and its most recent dated entry (top of the file, right after the "What this is" line) for the current verified state, then read EXECUTION_PLAN.md's "R2 backlog" section for what's next — as of the last update that's B-12 (Monthly progress-report PDF), unless a founder decision or a newer session has changed priorities (check "For the next session" at the very end of EXECUTION_PLAN.md first for the latest pick). Scope it as a new numbered step following this file's own convention (Goal/Why/Scope/DoD, exact file/line references, confirmed against the real schema by reading the code — not just this plan's one-line description), then execute it end to end: migration if needed (rehearse on `classstackr-staging` first, get explicit approval before any staging/production push), routes, client surface, RLS + contract tests, all seven gates from HANDOFF.md §2, then a live browser walkthrough against production per MASTER_PLAN.md §7's rule that money/interactive flows need one. Confirm before committing or pushing to `main` (auto-deploys via Vercel). Update this file's tracker, HANDOFF.md's gate line, and this "For the next session" section in the same pass before finishing.
