...
**Step 30 (C-05, D-06 parent-visible tutor-student threads) is in progress as of 2026-09-26**, the founder's pick for what follows Step 27 (Steps 28/29 stay on hold for Razorpay KYC). Built and gate-green on branch `feat/step-30-guardian-threads`: a parent can now read, but not post into, any DM between staff and their child; the database anchors every such thread to the child automatically; all three people see a disclosure; and, per founder direction the same day, a parent can now start a message to their child's tutors (previously they had no way to message a tutor at all). Two pre-existing messaging holes were closed in the same migration (anyone could post into any thread; class-channel posts were invisible to the class). **Next action: founder go-ahead to apply `20260926120000_guardian_thread_visibility.sql` to `classstackr-staging`**, then the three-account live walkthrough there, then PR merge and production apply with a second go-ahead. See Step 30's "Decisions" and "Status".
...
| 30 | C-05 parent-visible tutor-student threads (+ parents can message tutors) | R4 | 🔨 Built, gates green, on branch `feat/step-30-guardian-threads` ([PR #11](https://github.com/Sankaranakshar/Tuition-SaaS/pull/11)). Awaiting founder go-ahead for staging rehearsal, then live walkthrough |
...
**Definition of done.**
- [ ] A tutor cannot message a student without that student's guardian being able to read it. *(Built and proven at the database level by the RLS suite; live three-account walkthrough pending the staging rehearsal.)*
...
- [ ] Both sides see a disclosure. *(Built; live walkthrough pending.)*
