...
**Step 32 (C-07, activation analytics) is 🟡 built and green on all eight gates as of 2026-09-26**, the founder's pick after Step 31, on branch `feat/step-32-activation-analytics`, opened as a PR stacked on Step 31's (base `feat/step-31-playwright`) so the two don't conflict in this file. ClassStackr now records what people do in its own database (D-11: no third-party analytics), computes activation exactly as MASTER_PLAN.md §11 defines it, and shows rupees collected per org per month, the signup-to-activation funnel with per-beat drop-off, the weekly loop, retention cohorts, parent engagement and feature usage on a new Platform admin → Analytics tab. No minor's personal data can enter an event: the payload rule is written down and enforced in code. **Waiting on the founder, in order:** (1) apply the migration to staging (command in Step 32's Status), (2) the staging walkthrough listed there, (3) merge PR #13 then this PR, (4) apply the migration to production. Step 31's deliberate-failure CI check is still open on PR #13.
...
| 32 | C-07 activation analytics | R4 | 🟡 Built, all eight gates green 2026-09-26 (branch `feat/step-32-activation-analytics`, PR stacked on #13). Waiting on: migration applied to staging, staging walkthrough, merge, production migration |
...
**Definition of done.**
- [ ] The funnel reports a real number for at least one real org. *(Proven on PGlite and against staging's real data read-only: Demo Tuition Center computes as 1 of 10 sessions, ₹0, not activated. Needs the migration on staging, then production, to show on the page.)*
...
**Status: 🟡 built on branch `feat/step-32-activation-analytics`, all eight gates green; migration not applied anywhere.** `supabase db push --dry-run` on 2026-09-26 against both staging and production lists `20260926130000_product_analytics.sql` as the only pending migration. The founder applies it (this session's classifier refuses `supabase db push`, and the go-ahead rule says rehearse on staging first):

1. Staging, from the repo folder: `npx dotenvx run -f .env.staging -- sh -c 'supabase db push --db-url "$DATABASE_URL"'`
2. After the staging walkthrough and both merges (PR #13, then this PR), production: `supabase db push` (the repo is linked to production, `cwugpiernnwrhcximjwh`).
...
**Staging walkthrough, once the migration is on staging.**
