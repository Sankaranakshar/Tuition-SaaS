...
**Baseline as of 2026-09-23, after Step 26's cron GET/POST fix:** typecheck clean, 256 unit, 104 RLS, 351 contract, build (`dist/server.js` 237.0 KB), bundle 205.4 KB against a 260 KB budget, 18 of 18 API route mounts, `api/index.js` regenerated (236.2 KB). (The 2026-09-12 baseline of 249/106/353 included the since-parked B-19 referral loop's tests; those numbers are stale — this line reflects `main` as it actually stands.)

---

## Start here

**Step 27 (B-17, outbound comms router): code, tests, and staging rehearsal complete, on branch `feat/step-27-comms-router` ([PR #10](https://github.com/Sankaranakshar/Tuition-SaaS/pull/10)), not yet merged.** Picked up immediately after Step 26 closed for real (below). Everything D-10 unblocks is built and staging-verified: the provider-agnostic transport, template registry, outbox, retry/backoff/dead-letter sweep, delivery-status webhook, and all six producers (invoice raised, fee reminder, payment received, invite links, session reminder, absence alert) — all seven gates green locally, migration rehearsed clean on `classstackr-staging` with founder go-ahead, and the real remind flow walked live against a Vercel Preview build wired to staging (a throwaway invoice, a throwaway phone number, a queued message row confirmed in the database, all cleaned up afterward). **Next session should NOT start new work on this** until the founder gives explicit go-ahead to merge PR #10 to `main` and apply the migration to production (standing rule, this file's own header) — see Step 27's own "Status" line below for full detail. The DoD's one item that can't close regardless — a real message to a real phone — stays blocked on WhatsApp/SMS procurement, unstarted, multi-week, separate from engineering.
...
| 27 | B-17 outbound comms router | R3 | Code + tests + staging rehearsal complete ([PR #10](https://github.com/Sankaranakshar/Tuition-SaaS/pull/10)), awaiting founder go-ahead to merge; live send blocked on procurement |
...
- [x] All seven gates green locally on this branch (256+18 unit, 104+1 RLS, 351+13 contract, build, bundle, API bundle 18/18 — messaging routes live under the existing `/api/webhooks` and `/api/cron` mounts, so the count is unchanged). *(Not yet rehearsed on staging or merged — see Status.)*
