...
**Step 27 (B-17, outbound comms router): code and tests complete, on branch `feat/step-27-comms-router`, not yet merged.** Picked up immediately after Step 26 closed for real (below). Everything D-10 unblocks is built: the provider-agnostic transport, template registry, outbox, retry/backoff/dead-letter sweep, delivery-status webhook, and all six producers (invoice raised, fee reminder, payment received, invite links, session reminder, absence alert) — all seven gates green locally. **Next session should NOT start new work on this** until: (1) `20260923120000_messaging_outbox.sql` is rehearsed on `classstackr-staging`, (2) a PR is opened and a Vercel Preview build confirms it against staging, (3) the founder gives explicit go-ahead to merge and apply the migration to production (standing rule, this file's own header). See Step 27's own "Status" line below for full detail. The DoD's one item that can't close regardless — a real message to a real phone — stays blocked on WhatsApp/SMS procurement, unstarted, multi-week, separate from engineering.
...
| 27 | B-17 outbound comms router | R3 | Code + tests complete on branch, PR pending staging rehearsal + founder go-ahead; live send blocked on procurement |
...
**Status: code and tests complete on branch `feat/step-27-comms-router`, PR pending.** Built exactly what D-10 unblocks: the provider-agnostic transport abstraction (`server/utils/messaging/provider.ts` — a `ConsoleMessagingProvider` stand-in, same posture as B-08's no-live-Razorpay-payout-API), the template registry (six templates, in the plan's stated priority order), the outbox (`message_outbox`, migration not yet applied anywhere — see below), enqueue/retry/backoff/dead-letter (`server/routes/cron.ts`'s new `delivery-sweep` job, folded into one cron entry rather than two — Vercel Cron on Hobby only fires daily, so a separate enqueue-only job would still need this same sweep to send), the delivery-status webhook scaffold (inert until `MESSAGING_WEBHOOK_SECRET` exists, mirrors `/razorpay-platform`), and all six producers wired to enqueue (not yet to actually deliver): invoice raised, fee due reminder (Money's real remind button), payment received (manual + Razorpay webhook), invite link (parent + student invites), session reminder (daily sweep, 26h lookahead), absence alert. **Not done, and explicitly out of scope for this pass, per the plan's own "Blocked on" line:** picking a vendor, WhatsApp Business API onboarding, template approval, SMS DLT registration — all multi-week procurement, unstarted. **Next, before this can be called complete:** (1) rehearse `20260923120000_messaging_outbox.sql` on `classstackr-staging`, (2) open a PR and get a Vercel Preview build wired to staging, (3) founder go-ahead to merge to `main` and apply the migration to production — the standing rule this file states, unwaived. Once merged, the DoD's one open checkbox (a real message to a real phone) still can't close until procurement lands; everything else can.
