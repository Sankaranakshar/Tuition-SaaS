...
**Follow-up fix, 2026-09-27: Today, People, Inbox, Money's Outstanding, the parent portal and the Schedule wizard now use the org's calendar, not the viewer's** (branch `fix/browser-libs-org-tz`, built on `fix/revenue-trend-tz`, a Step 25 follow-up, no migration). The browser-side pure libs that PR #18's sweep left reading the ambient zone now take the org's zone explicitly: `src/lib/today.ts` (`daysOverdue`, `isSameDay`, `sessionsForDay`, the Pulse's month and week starts, the attention queue), `src/lib/format.ts` (`formatRelativeDays`) and `src/lib/schedule.ts` (`minutesSinceMidnight`, `pixelOffsetToTime`, `timeToPixelOffset`, `isOutsideAvailability`). Callers pass `organizations.timezone` from `useOrgTimezone()`, now its own file, `src/hooks/useOrgTimezone.ts`. **The Schedule page's writes were the real bug:** the Add Class wizard built a one-off's start as `new Date("yyyy-MM-dd")` (UTC midnight) plus a local `setHours()`, which lands on the previous day for any viewer west of UTC; dragging a recurring session wrote the template's `start_hour` from the viewer's `getHours()`; the tutor-availability prompt read the viewer's clock. All three now use the org's zone. **Deliberately unchanged:** the week grid's layout and every displayed time (`formatTime`, `formatDate`, date-fns labels) stay in the viewer's zone; whether staff abroad should see org time instead is a founder call. Twenty-one new unit tests, each failing against the old code; all eight gates green; walked live with a New York browser against an IST org on staging. Details in Step 25's second "Follow-up" note and HANDOFF.md §9.

**Follow-up fix, 2026-09-26: Money → Insights' revenue trend now buckets months in the org's timezone** (branch `fix/revenue-trend-tz`, a Step 25 follow-up, no migration). `revenueTrend()` in `src/lib/money.ts` bucketed payments with `getFullYear()/getMonth()`, i.e. the viewer's browser zone, and under `TZ=America/New_York` its unit test failed (a payment at 00:00 UTC on 1 July fell into June). **Not a regression:** the function and its test are unchanged since the Money workspace shipped on 2026-07-11, so the test already failed under New York when Step 25 closed; Step 25's New York check only ever ran `tests/unit/timezone.test.ts`, not the whole suite. `revenueTrend()` now takes the org's zone (required) and uses a new `monthKeyInZone()` in `shared/timezone.ts`; the Money page reads `organizations.timezone` through a new `useOrgTimezone()` hook. **New check:** `npm run test:tz` runs the whole unit suite under `UTC`, `Asia/Kolkata` and `America/New_York`, and CI's unit step now runs it instead of `npm test`. Seven gates green; the e2e gate is 6/7 on this branch alone because it ran at about 21:15 IST, inside journey 5's known failure window that `fix/leave-org-timezone` (PR #17) fixes, and 7/7 with that branch merged in locally. Details in Step 25's "Follow-up" note and HANDOFF.md §9.
...
| 25 | C-01 timezone model | R3 | ✅ Complete 2026-09-14; revenue-trend follow-up fixed 2026-09-26 (branch `fix/revenue-trend-tz`), browser-libs follow-up fixed 2026-09-27 (branch `fix/browser-libs-org-tz`), see "Start here" |
...
- [x] The unit suite passes identically under `TZ=UTC` and `TZ=Asia/Kolkata`. *(Also checked under `TZ=America/New_York` — a DST zone — for good measure. `tests/unit/timezone.test.ts`, 12 tests.)* *(Correction 2026-09-26: that New York check covered only `timezone.test.ts`. The whole suite failed one test under New York, `revenueTrend` in `money.test.ts`, from the close of this step until branch `fix/revenue-trend-tz`. `npm run test:tz` now checks all three zones on the whole suite, in CI.)*
...
**Follow-up (2026-09-26): Money's revenue trend read the ambient zone.** `revenueTrend()` in `src/lib/money.ts` (Money → Insights, shipped 2026-07-11) built its month buckets with `new Date(y, m, 1)` and bucketed payments with `getFullYear()/getMonth()`. In the browser that is the viewer's zone, not the org's; in the unit suite it is the test runner's, so `money.test.ts` failed under `TZ=America/New_York`. This step's sweep covered the scheduling path and its New York check ran only `timezone.test.ts`, so neither caught it. Fixed on branch `fix/revenue-trend-tz`: `revenueTrend(payments, now, zone, months)` takes the org's zone (required, no default), does its trailing-month arithmetic in UTC-only civil fields, and buckets each payment with the new `monthKeyInZone()` in `shared/timezone.ts`. `src/pages/Money.tsx` passes the org's zone from a new `useOrgTimezone()` hook in `src/hooks/useMoney.ts`, which reads `organizations.timezone` (readable by any member under `org_select`) and falls back to `Asia/Kolkata`. Tests: `tests/unit/money.test.ts` (the original test now passes `"UTC"`; month boundaries under `Asia/Kolkata` and `America/New_York` for the same two instants; the trailing window anchored on the org's current month; a year boundary) and `tests/unit/timezone.test.ts` (`monthKeyInZone` on both sides of UTC). New script `scripts/test-unit-all-tz.mjs` (`npm run test:tz`) runs the whole unit suite under `UTC`, `Asia/Kolkata` and `America/New_York` and fails if any zone fails; `.github/workflows/ci.yml`'s unit step runs it. **Still reading the ambient zone, not fixed here:** `src/lib/today.ts` (`daysOverdue`, the Pulse's month and week starts, `isSameDay`) and `src/lib/format.ts`'s relative-day helper. They run in the browser, so they use the viewer's zone rather than the org's; their tests pass in all three zones only because the fixtures sit away from midnight. Same class of bug, lower stakes for an all-IST customer base; worth its own pass. *(Done 2026-09-27, next note.)*

**Follow-up (2026-09-27): the browser-side libs read the ambient zone.** Fixed on branch `fix/browser-libs-org-tz`, built on `fix/revenue-trend-tz`. Every function below now requires the org's zone, no default:
- `src/lib/today.ts`: `isSameDay(a, b, zone)`, `sessionsForDay(sessions, day, zone)`, `daysOverdue(inv, now, zone)` (calendar days from the due date to the org's today), `buildPulse(invoices, sessions, now, zone)` (month from midnight on the 1st in the org's zone; Monday-anchored weeks walked as calendar days, so a DST week is 7 days and 1 hour, where the old `weekStart + 7 × 24h` dropped a Sunday 23:30 New York session on DST-end week), and `buildAttentionQueue(input, now, zone)`.
- `src/lib/money.ts` `groupOutstandingByPayer`, `src/lib/people.ts` `rankStudentsByAttention` and `src/lib/inbox.ts` `describeAnchor` take the zone through to `daysOverdue`.
- `src/lib/format.ts`: `formatRelativeDays(d, zone, now?)`, used by the parent portal's "next class" chip and session list.
- `src/lib/schedule.ts`: `minutesSinceMidnight(date, zone)`, `pixelOffsetToTime(dayStart, px, pxPerMinute, zone, step?)`, `timeToPixelOffset(date, pxPerMinute, zone)`, `isOutsideAvailability(slot, availability, zone)`; new `wizardFieldsInZone()` and `wizardStartInstant()` convert between an instant and the wizard's typed date and time on the org's clock.
- `shared/timezone.ts` gains `minutesSinceMidnightInZone()`, `startOfDayInZone()` and `civilDaysBetween()`.
- `useOrgTimezone()` moved from `src/hooks/useMoney.ts` to `src/hooks/useOrgTimezone.ts`; Today, People, Inbox, Money, ParentPortal and Schedule call it. Parents can read `organizations.timezone` too: `org_select` admits any member.

**`src/lib/schedule.ts:44` itself was test-only** (`pixelOffsetToTime` has no production caller), but its inline twins in `src/pages/Schedule.tsx` fed org-zone fields from the viewer's clock. Three write paths now use the org's zone: the wizard's one-off start (`new Date("yyyy-MM-dd")` is UTC midnight, so the old `setHours()` put an evening class on the previous day for any viewer west of UTC), the wizard's prefill from a grid drag or a "find a gap" slot, and the "all/future sessions" drag that writes a template's `start_hour`/`start_minute` (the server materializes those in the org's zone). The tutor-availability prompt reads the org's clock, matching the server's `/gaps`. **The week grid itself stays in the viewer's zone**, stated explicitly as `VIEWER_ZONE` in `Schedule.tsx`, because every displayed time in the app (`formatTime`, `formatDate`, date-fns labels) is the viewer's; moving grid columns to the org's days without moving displayed times would misplace blocks. Whether staff outside the org's zone should see org time everywhere is a product decision, not made here.

**Tests.** 21 new unit tests, all near midnight in IST or New York: `tests/unit/today.test.ts` (overdue days at 00:30 IST and 23:30 New York, a 00:15 IST session on the next IST day, the Pulse's month and Monday boundaries, the DST-end week, the queue's ageing), `tests/unit/format.test.ts` ("tomorrow" vs "today" across both zones, system clock pinned so only the zone differs), `tests/unit/schedule.test.ts` (minutes and pixel offsets in both zones, availability by the org's clock and weekday, the wizard's fields and typed instant) and `tests/unit/timezone.test.ts` (the three new helpers). With the old `today.ts`, `format.ts` and `schedule.ts` swapped back in, every new `today`/`format`/`schedule` boundary test failed under each of `UTC`, `Asia/Kolkata` and `America/New_York` (the two wizard-helper tests only because the helpers did not exist yet). Existing tests now pass `"UTC"` and `Z`-suffixed instants instead of local-time strings.

**Still reading the ambient zone, not fixed here:** `src/components/PayoutRuns.tsx:29` (default payout period, `new Date(y, m, 1)`), `src/components/ProgressReportDownload.tsx:23` (default report month), `src/pages/Money.tsx:741` (a +7-day default) and Today's greeting (`src/pages/Today.tsx`, viewer's hour, arguably correct as is). Separately, `POST /payouts/payout-runs` compares `session_start` against bare `YYYY-MM-DD` period bounds, which Postgres reads in the database session's zone (UTC on Supabase), not the org's; `tests/contract/payouts.test.ts`'s "sums unpaid earnings" test fails when the process runs in IST between 00:00 and 05:30 IST for the same reason (it passes under `TZ=UTC`, as in CI, and fails identically on `fix/revenue-trend-tz`). See HANDOFF.md §8.
