...
**Follow-up fix, 2026-09-26: leave windows now use the org's timezone** (branch `fix/leave-org-timezone`, a Step 25 follow-up, no migration). `shared/leave.ts`'s `leaveDateRangeToTimestampBounds` turned a leave's calendar dates into UTC-midnight bounds, so in an IST org a leave for the 27th covered 05:30 IST on the 27th to 05:30 IST on the 28th: that day's 00:00-05:30 sessions were missed and the next morning's were wrongly caught, in both `GET /leave/:id/affected-sessions` and the default set of `POST /leave/:id/reassign`. Pre-existing since Step 23 and missed by Step 25's sweep, because it built dates from a `T00:00:00.000Z` string rather than `setHours`. Found when Step 31's journey 5 failed at about 21:00 IST ("No scheduled sessions fall in this leave window"): it failed whenever the suite ran between about 21:00 and 02:30 IST, CI included. The helper now takes the zone explicitly and returns local-midnight bounds via `shared/timezone.ts`'s `zonedTimeToUtc`; `loadLeave` joins `organizations.timezone`. All eight gates green, e2e run at 21:10 IST, inside the old failure window. Details in Step 25's "Follow-up" note and HANDOFF.md §9.
...
| 25 | C-01 timezone model | R3 | ✅ Complete 2026-09-14; leave-window follow-up fixed 2026-09-26 (branch `fix/leave-org-timezone`, see "Start here") |
...
**Follow-up (2026-09-26): the leave window was missed.** Step 23's `leaveDateRangeToTimestampBounds` (`shared/leave.ts`) still read leave dates as UTC days. This sweep looked for `setHours`/`getHours`-style ambient-zone reads, and this helper had none: it built `` new Date(`${date}T00:00:00.000Z`) ``, which is zone-explicit, just the wrong zone. Fixed on branch `fix/leave-org-timezone`: the helper takes the org's zone (required, no default, same rule as `shared/timezone.ts`) and `server/routes/leave.ts`'s `loadLeave` joins `organizations.timezone`. Tests: `tests/unit/leave.test.ts` (IST bounds; a 00:30 IST session on the leave day is covered and one the next day is not; a UTC org; a New York DST day) and `tests/contract/leave.test.ts` (the same two sessions through the real affected-sessions and reassign routes, plus an org switched to UTC). The new IST contract test fails against the old helper. Unit suite passes identically under `TZ=UTC` and `TZ=Asia/Kolkata`; the leave tests also pass under `TZ=America/New_York`.
