...
3. **First end-to-end walkthrough — tutor signup through onboarding now confirmed working (2026-07-10, HANDOFF §14).** Getting there surfaced and fixed three real bugs, not one: (a) Vercel never registered the API as a serverless function at all — it was build-time-generated and gitignored, invisible to Vercel's pre-build function scan; (b) the server-side `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` env vars pointed at a completely different Supabase project than the one actually migrated and used for login, so every JWT verification 401'd against the wrong project's JWKS; (c) a stale-deployment false-positive in the verification process itself. Full writeup in HANDOFF §14 — read it before touching Vercel deploy config or Supabase env vars again. **Immediately hit next, blocking session booking: no UI anywhere creates a `courses` row (Tech Debt #19)** — a brand-new org has an empty course dropdown with no way to populate it short of a manual Supabase Table Editor insert. **Remaining for this blocker:** build a minimal courses UI (or unblock manually per-org for now) → add student → book session → confirm student sees own session (the exact §11.4 regression) → Today attendance → invoice accrual → manual payment → ledger. Realtime subscriptions specifically are still unverified against a live server.
...
| 16 | **Student self-onboarding has no way to join an organization at all.** `role_type: 'student'` never triggers `loadUser`'s bootstrap path (only tutor/admin do), and the `parentCode` field the UI collects is never sent anywhere or used to resolve a membership — it's dead state. Currently surfaces a clear "ask your tutor for an invite" message (was a raw constraint-violation crash before this pass) rather than actually working. | High | Any student who signs up standalone (not via the parent-portal invite flow) cannot complete onboarding | 2-3 ed | A real student join-by-code (or staff-invite) flow is needed, mirroring the parent `parent_invites` pattern | Epic 10 pattern (`server/routes/parents.ts`) as a template |
...
| 19 | **No UI anywhere creates a `courses` row.** Booking a class (Calendar.tsx "Add Class") requires selecting a course from a dropdown populated by `SELECT * FROM courses` — but the entire codebase only ever reads that table, never inserts into it. A brand-new org has zero courses and the dropdown is permanently empty with no way to add one from the UI. Found live: booking blocked on a fresh org until a course was inserted manually via the Supabase Table Editor. | Critical | Every new org is blocked from booking any class at all until someone manually inserts a database row — this is the first thing a real pilot user would hit | 1-2 ed | A minimal "manage courses" UI (a simple list + create form, or an inline "+ New course" option in the Add Class modal) — schema is trivial (`organization_id`, `program_id` optional, `name`) and RLS already permits org-admin writes (`courses_write`), so this is pure frontend work | none |
| 20 | **Add Class modal has no UI control for pricing model or fee amount at all — every class is silently created as Monthly/₹0.** `Calendar.tsx` declares `pricingModel`/`feeAmount` React state (defaulting to `MONTHLY`/`0`) but never wires an `onChange` or input to either — `setPricingModel`/`setFeeAmount` are called nowhere except their own `useState` initializer. Confirmed live: every `class_templates` row created through the app has `fee_amount: 0.00`. This silently disables the entire attendance→billing path, since `server/routes/billing.ts`'s attendance handler only bills when `pricing_model === 'PER_SESSION'` (`nowBillable = perSession && BILLABLE.has(status)`) — a Monthly-priced class (the only kind the UI can produce) marks attendance with `billed: false` forever and never debits a wallet or accrues an invoice. This is not a bug in the billing logic itself (Monthly pricing is meant to be handled by a separate monthly batch-invoicing job — see Phase 2 "Monthly batch drafting job", not yet built) — it's that **the UI provides no way to create the one pricing model (`PER_SESSION`) the built billing path actually supports.** Confirmed unblockable only via direct SQL (`update class_templates set pricing_model = 'PER_SESSION', fee_amount = ...`). | Critical | The core wedge-demo money loop (attendance → invoice) cannot be exercised through the product at all today — every class ever created via the UI is billing-inert | 1-2 ed | Add pricing model (radio/select: Per Session / Monthly) + fee amount (number input) fields to the Add Class modal, wired to the existing `pricingModel`/`feeAmount` state that's already there and already sent in `templateData` | none — purely a missing form control |
