...
3. **First end-to-end walkthrough — tutor signup through onboarding now confirmed working (2026-07-10, HANDOFF §14).** Getting there surfaced and fixed three real bugs, not one: (a) Vercel never registered the API as a serverless function at all — it was build-time-generated and gitignored, invisible to Vercel's pre-build function scan; (b) the server-side `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` env vars pointed at a completely different Supabase project than the one actually migrated and used for login, so every JWT verification 401'd against the wrong project's JWKS; (c) a stale-deployment false-positive in the verification process itself. Full writeup in HANDOFF §14 — read it before touching Vercel deploy config or Supabase env vars again. **Remaining for this blocker:** add student → book session → confirm student sees own session (the exact §11.4 regression) → Today attendance → invoice accrual → manual payment → ledger. Realtime subscriptions specifically are still unverified against a live server.
