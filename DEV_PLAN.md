...
_Rewritten 2026-07-25, updated 2026-08-02 against commit `fc18383` plus an uncommitted Tech Debt #7 token-styling pass. Every status below was verified by re-running all gates and checking the claims in code, not inherited from the previous plan._
...
| Route contracts | `npm run test:contract` | 195 | The real Express app through supertest against PGlite-backed shims. Covers the auth matrix (401/403/200/409/422) that the RLS layer structurally cannot see, plus webhook HMAC verification. Runs in CI now (was local-only). |
